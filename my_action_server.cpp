...
const double g_spin_speed = 0.3925; // set yaw rate to this value, e.g. 1 rad/s
...
bool callback(example_ros_service::PathSrvRequest& request, example_ros_service::PathSrvResponse& response)
{
    ...
    for (int i=0;i<npts;i++) { 
        ...
        double dx,dy;
	dx=pose_desired.position.x-g_current_pose.position.x;//Horizontal component of desired distance
	dy=pose_desired.position.y-g_current_pose.position.y;//Vertical component of desired distance
	travel_distance=sqrt(dx*dx+dy*dy);//Magnitude of the distance
	global_x=global_x+dx;//actual x coordinate wrt starting position
	global_y=global_y+dy;//actual y coordinate wrt starting position
        ...
        ROS_INFO("GLOBAL CO-ORDINATES (x,y)=(%f,%f)",global_x,global_y);//Actual position with respect to original position.
        
        
        yaw_desired = convertPlanarQuat2Phi(pose_desired.orientation); //from i'th desired pose
        double orientation;//Initial angle
     
	spin_angle=atan2(dy,dx)-orientation;//Angle of rotation
	ROS_INFO("spin angle=%f",spin_angle);
        do_spin(spin_angle); // carry out this incremental action
       
        orientation = atan2(dy,dx); // actual angle wrt to current robot position
        
        
        do_move(travel_distance);  // move forward 
}
  return true;
}
